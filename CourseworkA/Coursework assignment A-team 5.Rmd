...
Here below we plot the means of each class using plotmeans from the package gplots.  We can see that the mean for Beyonce is `r meanBeyonce`, for Madonna is `r meanMadonna` and for Kanye West it is `r meanWest`. Where a lower value means that the sentiment analysis is more negative.
```{r}
...
According to results here above: Madonna vs West has significance that is adjusted to bonferroni of $`r BonferroniP[4]`$,Madonna vs Beyonce has significance that is adjusted to bonferroni of $`r BonferroniP[1]`$ and Beyonce vs West has significance that is adjusted to bonferroni of $`r BonferroniP[2]`$. We can see that the bonferroni adjustment leads to all te p-values to be higher than the p-value obtained in the Anova test above. Since the value for Beyonce and Madonna is rather close to 1 it tells us that there really is not much difference on the sentiment for the two. But th other two pairs still have a significant difference, that it the sentiment is significantly different between the two persons in the pair.
